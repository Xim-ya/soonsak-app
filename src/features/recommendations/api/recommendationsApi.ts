  ...
  getPersonalizedCurationVideos: async (limit: number = 10): Promise<CurationVideoModel[]> => {
    ...
    return ((data as CurationVideoRow[]) ?? []).map(
      (item): CurationVideoModel => ({
        ...
        // Optional 필드들은 조건부로 추가
        ...(item.thumbnail_url && { thumbnailUrl: item.thumbnail_url }),
        ...(item.runtime && { runtime: item.runtime }),
        ...(item.channel_id && { channelId: item.channel_id }),
        ...(item.channel_name && { channelName: item.channel_name }),
        ...(item.channel_logo_url && { channelLogoUrl: item.channel_logo_url }),
        ...(item.poster_path && { posterPath: item.poster_path }),
        ...(item.release_date && { releaseDate: item.release_date }),
        ...(item.genre_ids && { genreIds: item.genre_ids }),
      }),
    );
  },
};
